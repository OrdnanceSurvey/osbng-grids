...
The repository also contains the Python [script](osbng-grids/scripts/create_osbng_grids.py) used for data generation and a Jupyter [notebook](osbng-grids/examples/notebooks/create_osbng_grids_examples.ipynb) demonstrating custom grid creation with the [`osbng`](https://github.com/OrdnanceSurvey/osbng-py) Python package.
...
Grid data at finer BNG resolutions below 1km, or for custom geographic extents, can be generated by applying the same methods with different parameters. See the notebook [example](osbng-grids/examples/notebooks/create_osbng_grids_examples.ipynb) of creating custom BNG grid datasets.
